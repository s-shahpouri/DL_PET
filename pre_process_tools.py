...
def resample_to_size(input,output,new_size,default_value=-1000.0):
    image = sitk.ReadImage(input)
    
    # Compute the new spacing    
    new_size = [128,128,128]
    spacing = [sz*old_sz/new_sz for sz,old_sz,new_sz in zip(image.GetSize(),image.GetSpacing(),new_size)]
    ...
    resampled_image = sitk.Resample(image, new_size, transform, interpolation, 
                                    image.GetOrigin(), spacing, image.GetDirection(),
                                    default_value, output_pixel_type)
    
    # Save the resampled image
    sitk.WriteImage(resampled_image, output, True)
